...
export const HousePointsRow: React.FC<IndependentHouseProps> = ({
    colorName,
    houseImage,
    amountPoints,
    houseName,
    ...
    return (
        <div className="grid place-items-center min-w-screen">
            <div
                className={`shadow-lg flex flex-row h-14 w-full rounded-xl bg-gradient-to-r hover:outline outline-4 from-${colorName}-400 to-${accentColor}-700 outline-${accentColor}-900 shadow-${colorName}-500/50 outline-${accentColor}-900"}`}
            >
                ...
                <p
                    className={`basis-3/5 font-stretch-150% font-mono font-bold text-${fontSize}xl flex-1 basis-16`}
                >
                    {houseName}
                </p>
            ...
            <div className="">
                {houseData.map((house, index) => (
                    <HousePointsRow
                        ...
                        amountPoints={house.amountPoints}
                        colorName={house.colorName}
                        fontSize={house.fontSize}
