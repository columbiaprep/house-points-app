...
async function setLeaderboardsLogic() {
    ...
    try {
        ...
        functions.logger.info(
            "Wrote top overall leaderboard.... Writing to database now....",
        );
        const topOverallRef = db.collection("leaderboards").doc("topOverall");

        topOverallRef.set({ topOverall });
        functions.logger.info(
            "Succesfully wrote top overall leaderboard to database... Writing house leaderboards now....",
        );
        data.houses.forEach((house) => {
            const houseIndividuals: Array<IndividualDocument> =
                data.individuals.filter(
                    (individual) => individual.house === house.name,
                ) as IndividualDocument[];
            const houseLeaderboard = houseIndividuals
                .sort((a, b) => b.totalPoints - a.totalPoints)
                .slice(0, 4);
            const houseLeaderboardRef = db
        ...
        functions.logger.info(
            "Successfully wrote house leaderboards to database... Completing operation now...",
        );
        functions.logger.info(
            "Operation completed successfully - Created by Sam Zack",
        );
    } catch (error) {
